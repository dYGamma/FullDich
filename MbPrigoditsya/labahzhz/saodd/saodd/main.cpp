...
void PrintTree(Tree* p, int level = 0, bool printHeights = false)
{
	if (p)
	{
		PrintTree(p->Right, level + 1, printHeights); //сначала выводим левую ветвь, потому она выше правой
		...
		PrintTree(p->Left, level + 1, printHeights); //потом выводим правую ветвь
