...
inline bool operator< (const log_record& lhs, const log_record& rhs) { return std::stoi(rhs.room_code.substr(2, 3)) > std::stoi(lhs.room_code.substr(2, 3)); }
...
inline bool operator< (const room& lhs, const room& rhs) { return std::stoi(rhs.room_code.substr(2, 3)) > std::stoi(lhs.room_code.substr(2, 3)); }
...
auto delete_from_subtree(std::shared_ptr<tree_node>& base, const std::string& room_code) {
    ...
    size_t bf = base->get_balance_factor();
...
int32_t bm_search(const std::string& haystack, const std::string& needle) {
    size_t abc_size = std::numeric_limits<char>::max();
    std::vector<size_t> char_table{};

    char_table.resize(abc_size, needle.size());

    for (size_t i = 0; i < needle.size(); i++) {
        char_table[needle[i]] = needle.size() - 1 - i;
    }
    ...
    for (size_t i = needle.size() - 1; i < haystack.size();) {
        ...
        i += offset_table[needle.size() - 1 - j] > char_table[haystack[i]] ? offset_table[needle.size() - 1 - j] : char_table[haystack[i]];
    }
    return -1;
