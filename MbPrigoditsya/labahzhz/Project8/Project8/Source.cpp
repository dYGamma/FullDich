...
{
	while (!isEmpty(backup)) // Пока бекап не пустой 
	{
		Task* task = getTaskFront(backup); // Получить задачу из бекапа
		bool isAddSuccess = pushToQueue(queueF, task); // Отправить задачу из бекапа в очередь F 
		if (!isAddSuccess) // Если добавление в очередь прошло не успешно
		{
			addTaskFront(backup, task); // Вернуть задачу на хранение, чтобы потом попробовать ее еще раз добавить в очередь
		}
	}
...
void processorLoop(TaskList*& IncomingTask)
{
	...
	while (true)
	{
		fromTaskGeneratorToQueue(IncomingTask, backup, queueF, timer); // Вызов функции продвижения задачи из генератора в очередь
		...
		if (!isEmpty(backup)) // Если бекап не пустой
		{
			std::cout << "Задания не попавшие в очередь и оставшиеся на хранение" << std::endl;
			showStruct(backup); // Вывести содержимое бекапа
		}
...
void exec()
{
	...
	{
		TaskList* tl = nullptr;
		addTaskBack(tl, new Task{ "Task 1", });
		addTaskBack(tl, new Task{ "Task 2",});
		addTaskBack(tl, new Task{ "Task 3",});
